...
function addExtrudedPolygonSideFacesWithDup(vEdges, uvsEdges, uvs, arrVertices, vertices, indices, length, offset, count, isClockWise) {
    ...
    for (let i = offset, j = startIndice; i < offset + count - 1; ++i, ++j) {
        if (isClockWise) {
            ...
            const uvCoordA = new itowns.Coordinates('EPSG:4978', vertices[indices[j - 5] * 3], vertices[indices[j - 5] * 3 + 1], vertices[indices[j - 5] * 3 + 2]).as('EPSG:4326');
            const uvCoordB = new itowns.Coordinates('EPSG:4978', vertices[indices[j - 4] * 3], vertices[indices[j - 4] * 3 + 1], vertices[indices[j - 4] * 3 + 2]).as('EPSG:4326');
...
function featureToExtrudedPolygon(feature, options) {
    ...
    for (const geometry of feature.geometry) {
        ...
        for (let i = 0; i < triangles.length; i++) {
            ...
            var c = new itowns.Coordinates('EPSG:4978', posWGS.x, posWGS.y, posWGS.z).as('EPSG:4326'); // Geocentric coordinates
        ...
        //console.log("verticesRoofWithDup", verticesRoofWithDup);
